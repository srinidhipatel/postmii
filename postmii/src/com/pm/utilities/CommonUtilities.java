...
public class CommonUtilities {
	...
	public void loadPropertyFiles(String PropertiesFilePath) throws Exception {
		...
		props.load(fileIn);
		try {
			oAndUtil.ufElementDisplayed(driver, "Home_Contextualmenu_SideMenu_NotificationCount");
	...
	public boolean ufVerifyTextInsideList(List<WebElement> liInput, String sVerifyingText) throws Exception {
	...
	public void ufUserErrorMessage(boolean bExpectedBoolean, boolean ActualBoolean, String strExceptionSummary)
			throws Exception {
		if (bExpectedBoolean != ActualBoolean) {
			oTest.sErrorMessage = oTest.sErrorMessage + "," + strExceptionSummary;
	...
	public void takeScreenShot(AppiumDriver<WebElement> driver, String className) throws IOException {
	...
	public void scrollUp(AppiumDriver<WebElement> driver) throws InterruptedException {
	...
	public boolean ufClickWithList(AppiumDriver<WebElement> driver, List<WebElement> liSideMenuList,
			String sTextToClick) throws Exception {
		boolean bRes_flag = false;
	...
	public boolean CheckAppPresence(AppiumDriver<WebElement> driver) throws Exception {
		boolean bRes_flag = false;
		if (TestBase.sClassName.contains("PH_")) {
		...
		if (bRes_flag)
			...
			Thread.sleep(5000);
			oAndUtil.launchApp();
			oAndUtil.ufWaitForElementDisplayed(driver, "WelcomeScreen_TextGetStarted", 15);
		}
	...
	public void SendEmailAfterTestSuite() throws Exception {
		...
		try {
			...
			String zipfile = System.getProperty("user.dir") + "/test-output/screenshots.zip";
			/*
			 * //System.get("curr.dir") DataSource source = new FileDataSource(filename);
	...
	public boolean ufVerifyContainsTextInsideList(List<WebElement> liInput, String sVerifyingText) throws Exception {
		boolean bRes_Flag = false;
		for (int i = 0; i < liInput.size(); i++) {
	...
	public void ufClickWithContainsElementList(AppiumDriver<WebElement> driver, List<WebElement> liSideMenuList,
			String sTextToClick) throws Exception {
		for (int i = 0; i < liSideMenuList.size(); i++) {
	...
	public void minScrollUp(AppiumDriver<WebElement> driver) throws InterruptedException {
	...
	public void mediumScrollUp(AppiumDriver<WebElement> driver) throws InterruptedException {
	...
	public boolean ufVerifyelementWithinList(AppiumDriver<WebElement> driver, List<WebElement> liSideMenuList,
			String sTextToVerify) throws Exception {
		boolean bRes_flag = false;
		for (int i = 0; i < liSideMenuList.size(); i++) {
			log.info(liSideMenuList.get(i).getText());
