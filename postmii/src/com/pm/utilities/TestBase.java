...
public class TestBase {
	...
	public void TriggerDependencies() throws Exception{
		...
		if(Constants.sConstEnvironment.equalsIgnoreCase("Stage"))
			oCommUtil.loadPropertyFiles(System.getProperty("user.dir")+"/src/Properties/ObjectRepositary_Stage.properties");
		else
		...
		/*oJsonLocalization = oCommUtil.ReadJsonFileGetJsonObject(System.getProperty("user.dir")+"/src/TestData/LocalisationSupport.json");
		sLocalization=System.getProperty("localization");
		log.info("Localisation set to : "+sLocalization);
		if(sLocalization.equalsIgnoreCase(sLocalEnglish))
			oJsonLocalizationData = oJsonLocalization.getJSONObject(sLocalEnglish);
		else
		{
			oJsonLocalizationData = oJsonLocalization.getJSONObject(sLocalIndonasia);
		...
		//boolean bRes=oAndUtil.ufWaitForElementDisplayed(driver, "Home_Contectualmenu", 15);
